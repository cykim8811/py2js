        ...
        for i, pr in enumerate(self.param):
            if type(pr) == Value:
                if pr.op == "listcomp":
                    ...
                    self.param[i] = Value("var", ["_listcomp_" + str(code.temp_var)])
                    code.temp_var += 1
                else:
...
class Structure:
    ...
    def tostr(self, indent):
        ...
        if structure == "else :": structure = "else {"
        if structure == "for A in V :": structure = "for ( var A in V ) {"
        if structure == "while W :": structure = "while ( W ) {"
        if structure == "def S L :": structure = "function S L {"
        if structure == "class S L :": structure = "function S L {"
        if structure == "class S :":
            structure = "function S L {"
        ...
        _ret += _str
        for ln in self.line:
            _ret += "\n" + ln.tostr(indent + 1) + (";" if type(ln) is Value else "")
        _ret += "\n" + " " * 4 * indent + "}"
        return _ret
    def tojs(self, parent, code):
        self.var_list=[]
    ...
    if code_str[i] in char_list:
        ...
        while i < len(code_str):
            if (not mline) and code_str[i] == quote:
                ...
                if len(code_str) <= i + 1: show_error("Unexpected end of string")
        ...
        if i == 0: show_error("Misused indent")
        if code_str[i - 1] == "\n":
            while code_str[i] == " ":
                i += 1
                _ret += " "
...
def parse_statement(ind, indent):
    statement_list = ["if", "elif", "else", "for", "while", "def", "class", "with", "try", "except", "finally"]
    i = ind
...
def parse_structure(ind, indent):
    ...
    for st_l in structure_list:
        ...
        if i < len(code_str) and not code_str[i] == "\n":
            pv, i = parse_line(i)
            if not pv: show_error("Invalid Syntax")
            while code_str[i] == " ": i += 1
            if code_str[i] == "#":
                while i < len(code_str) and code_str[i] != "\n":
                    i += 1
...
def parse_var(ind):
    ...
    def find_word(ind):
        ...
        def search_number(_ind):
            ...
            if ps.isdigit():
                ...
                if code_str[i] == "-": return False, _ind
                sn = search_number(i)
                if sn[0]:
    ...
    def find_attach(ind):
        ...
        def search_slicing(_ind):
            ...
            if pf is not False: return Value("slicing", [None, None]), i
            pf, i = parse_format(_ind, "[ W : ]".split())
            if pf is not False: return Value("slicing", [pf[0], None]), i
            pf, i = parse_format(_ind, "[ : W ]".split())
            if pf is not False: return Value("slicing", [None, pf[0]]), i
            pf, i = parse_format(_ind, "[ W : W ]".split())
            if pf is not False: return Value("slicing", pf), i
            return False, _ind
        def search_calling(_ind):
            def get_arguments(__ind):
                pf, i = parse_string(__ind)
                if pf != "(": return False, __ind
                listdata = []
                while i < len(code_str):
                    while code_str[i] in " \n": i += 1
                    pf, ii = parse_string(i)
                    ...
                    if pf == ",":
                        ...
                        show_error("Invalid syntax parsing tuple")
                show_error("Unexpected End in parsing tuple")
...
def parse_value_without_baretuple(ind, doassign=True):
    def find_statement(ind):
        ...
        for kl in keyword_list:
            ...
            if pf is False: continue
            return Value("_".join(kl), pf), i
        return False, ind
    def find_operation(ind):
        operator_list = ((
                             "=,+=,-=,*=,/=," if doassign else "") + "==,!=,<=,>=,is,is not,in,not in,+,-,*,/,>,<,|,**,//,>>,<<,and,or,%,^,&,if,else").split(
            ",")
        ...
        for e_operator in operator_list:
            ...
            if e_operator == ["if_else"]:
                while "if" in p_list and "else" in p_list:
                    ...
                    p_list[i1 - 1:i2 + 2] = [Value("ternary", [p_list[i1 - 1], p_list[i1 + 1], p_list[i2 + 1]])]
            while any([k in p_list for k in e_operator]):
                i = min([p_list.index(k) if k in p_list else 1000 for k in e_operator])
                if e_operator[0] == "**":
                ...
                if not (type(p_list[i - 1]) is Value and type(p_list[i + 1]) is Value):
                    return False
                    # show_error("Error parsing operators")
                p_list[i - 1:i + 2] = [Value(p_list[i], [p_list[i - 1], p_list[i + 1]])]
        if len(p_list) != 1:
            return False
        return p_list[0]
